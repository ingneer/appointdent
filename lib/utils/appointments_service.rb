...
class AppointmentsService
  class << self
    ...
    def perpetual_timeslots
      {}.tap do |ha|
        search_dates.each do |date|
          from = set_time(date, lunch_start)
          to = set_time(date, lunch_end)
    ...
    def appointment_time_ranges
      {}.tap do |ha|
        search_dates.each do |date|
          from = set_time(date, day_start)
          to = set_time(date, day_end)
          ha[date] = intervals_in_minutes(from, to)
        end
      end
    end
    ...
    def set_time(datetime, hours_minutes)
      DateTimeService.set_hours_minutes(datetime, hours_minutes)
    end

    def intervals_in_minutes(from, to)
      DateTimeService.set_date_intervals(from, to, INTERVAL, MIN_IN_DAY)
    end
    ...
    def any_blocked?(booked_appointments, free_timeslot)
      ...
      booked_appointments.any? do |booked_timeslot|
        ...
        day_ends = set_time(free_timeslot.to_date, day_end)
    ...
    def booked_values(booked_timeslots, key)
      [].tap do |arr|
        booked_timeslots.each_index do |index|
          arr.push(booked_timeslots.dig(index, key))
        end
      end.compact.flatten
    end
    ...
    def grouped_by_date
      formatted_timeslots.group_by { |weekly_entry| weekly_entry[:from].to_date }
    end
    ...
    def search_dates
      DateTimeService.format_search_dates(SEARCH_RANGE)
    end
